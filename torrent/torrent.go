...
type TorrentFile struct {
	Announce      string                 // URL of the main tracker
	AnnounceList  [][]string             // List of alternative trackers (each sublist is a tier)
	Comment       string                 // Optional comment about the torrent
	CreatedBy     string                 // Name of the program that created the torrent
	CreationDate  int64                  // Creation time (Unix timestamp)
	Encoding      string                 // Character encoding used in text fields
	Info          TorrentInfo            // Core metadata about the files being shared
	Nodes         [][]interface{}        // DHT bootstrap nodes (IP and port)
	URLList       []string               // List of Web Seed URLs (HTTP/FTP sources)
	HTTPSeeds     []string               // Legacy HTTP seed URLs
	Publisher     string                 // Name of the publisher (optional)
	PublisherURL  string                 // URL of the publisher (optional)
	Source        string                 // Source identifier for private torrents
	Signature     string                 // Digital signature (if present)
	Custom        map[string]interface{} // Non-standard/custom fields (not encoded)
	Peers         []Peer                 // List of peers participating in the download
	PeersMutex    sync.Mutex             // Mutex for synchronizing access to Peers
	PieceLength   int64                  // Length of each piece in bytes
	NumPieces     int                    // Total number of pieces
	PieceHashes   [][20]byte             // SHA-1 hashes of each piece
	Downloaded    []bool                 // Bitfield indicating downloaded pieces
	DownloadMutex sync.Mutex             // Mutex for synchronizing download state
	Files         []FileInfo             // Local file info (paths, offsets, handles)
}
...
type TorrentInfo struct {
	PieceLength int64                  // Length of each piece in bytes
	Pieces      string                 // Concatenated SHA-1 hashes of pieces
	Name        string                 // Name of the file or directory
	Length      int64                  // Total length (for single-file torrents)
	Files       []TorrentFileEntry     // List of files (for multi-file torrents)
	MD5Sum      string                 // Optional MD5 checksum of the file
	Private     int                    // 1 if this is a private torrent
	Source      string                 // Optional source field
	MetaVersion int                    // BEP-9: versioning for future compatibility
	FileTree    map[string]interface{} // BEP-47: file tree representation
	PieceLayers map[string]string      // BEP-47: used for Merkle root trees
	PiecesRoot  string                 // BEP-47: root hash of the Merkle tree
	Custom      map[string]interface{} // Non-standard/custom fields (not encoded)
	InfoHash    [20]byte               // SHA-1 hash of the bencoded Info dictionary
}
